...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function showLoading(isLoading) {
        if (!buttonText || !loadingSpinner || !submitButton) return;
    ...
    function formatNumberInput() {
        if (!this.value || this.name === 'rate' || this.name === 'tenure') return;
    ...
    function initializeResultsSection() {
        ...
        // Add staggered animations for breakdown cards
    ...
    function initializeChart() {
        ...
        if (!chartElement || !resultsSection) return;
        
        const ctx = chartElement.getContext('2d');
        
        // Get data attributes
        const principalAmount = parseFloat(chartElement.dataset.principal || 0);
        const interestAmount = parseFloat(chartElement.dataset.interest || 0);
        ...
        if (amount && rate && tenure && amount > downPaymentValue) {
            ...
            try {
                ...
                if (!isNaN(emi) && emi > 0 && buttonText) {
                    buttonText.innerHTML = `Calculate EMI (~₹${formatNumber(Math.round(emi))})`;
                }
            } catch (e) {
